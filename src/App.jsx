...
export default function App() {
  ...
  const updateRecents = (newClass) => {
    ...
    const isAlreadyIn = stored.some(item => item.fullCode === newClass.fullCode);
    if (!isAlreadyIn) {
  ...
  const renderScreen = () => {
    switch (screen) {
      case 'options':
        return <OptionsScreen selectedClass={selectedClass} onEnterBuilding={() => setScreen('interior')} onBack={handleBack} />;
      case 'interior':
        return <InteriorScreen selectedClass={selectedClass} onBack={() => setScreen('options')} />;
      case 'home':
      default:
  ...
  return (
    <main className="bg-gray-50 min-h-screen font-sans text-gray-900 flex items-center justify-center">
      <div className="w-full max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl my-4 relative min-h-[600px] flex flex-col justify-center">
        {renderScreen()}
      </div>
    </main>
  );
